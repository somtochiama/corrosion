...
impl<I: MessageId, P: Payload<Node = N>, N: NodeId> PlumtreeState<I, P, N> {
    ...
    pub fn receive_ihave(&mut self, msg: IHaveMsg<I, N>, rt: &mut impl Runtime<I, P, N>) {
        ...
        if new_missing > 0 {
            debug!(?sender, count, new_missing,
                   total_missing = self.missing.len(), "local={:?} sender={:?} receive_ihave → scheduled grafts", self.local_id, sender);
        }
    }
    ...
    pub fn receive_prune(&mut self, msg: PruneMsg<I, N>, rt: &mut impl Runtime<I, P, N>) {
        debug!(?msg.triggered_by, "local={:?} sender={:?} receive_prune", self.local_id, msg.sender);
