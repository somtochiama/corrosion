...
use corro_types::broadcast::{BroadcastId, BroadcastV1, ChangeSource, ChangeV1, PlumTreeMsg, PlumtreeInput};
...
struct CorrosionPlumtreeRuntime<T: TransportExt> {
    ...
    timer_spawner: TimerSpawner<plum_foca::Timer<BroadcastId>>,
...
impl<T: TransportExt + Clone + Send + 'static> plum_foca::Runtime<BroadcastId, ChangeV1, ActorId>
...
pub async fn plumtree_loop<T: TransportExt + Clone + Send + 'static>(
    ...
    let mut state: PlumtreeState<BroadcastId, ChangeV1, ActorId> =
                ...
                Some(PlumtreeInput::Wire(msg)) => {
                    match msg {
                        PlumTreeMsg::Gossip(g) => {
                            if g.sender == rt.agent.actor_id() {
                                state.broadcast(g.id, g.payload, &mut rt);
                            } else {
                                state.receive_gossip(g, &mut rt);
                            }
                            stats.0.fetch_add(1, Ordering::Relaxed);
                        }
                        PlumTreeMsg::IHave(ih) => {
                            state.receive_ihave(ih, &mut rt);
                            stats.1.fetch_add(1, Ordering::Relaxed);
                        }
                        PlumTreeMsg::Graft(g) => {
                            state.receive_graft(g, &mut rt);
...
mod tests {
    ...
    async fn test_plumtree_broadcast_spread() -> eyre::Result<()> {
        ...
        println!("gossip duplicates: {:.1}", total_gossip as f64 / total_expected as f64 * 100.0);
